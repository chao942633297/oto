...
class Wechat{
	...
	public static function transfersqiye($openid,$money,$cert,$sslKey,$desc='企业转账',$check_name='NO_CHECK',$re_user_name=''){
		...
		# 初始化企业转账类
		$transfers = new Transfers(config('wechat')['appid'], config('wechat')['mch_id'], config('wechat')['pay_key']);
	...
	public static function app_pay($pay_order_num,$order_name,$order_price){
		...
		$unifiedorder = new Unifiedorder(config('wechat')['open_appid'],config('wechat')['open_mchid'],C('wechat')['openid_pay_key']);
		# 设置商品标题
...
//		dump($data);exit;
	...
	public static function Cash($openid,$money,$cert,$sslKey,$num=1,$send_name="",$action_name='恭喜发财',$description='红包可立即提现',$wishing='恭喜发财'){
		...
		# 初始化红包类
		self::$cash = new Cash(config('wechat')['appid'], config('wechat')['mch_id'], config('wechat')['pay_key']);
	...
	public static function addEvent($event,$callback,$value=[]){
		...
		isset($result['Event']) ? : $result['Event']='';
